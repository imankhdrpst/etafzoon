...
public class StepView extends View {
    ...
    @ColorInt
    private int selectedCircleColor2;
    @ColorInt
    private int selectedCircleColor3;
    @Dimension
    private int selectedCircleRadius;
    @Dimension
    private int selectedCircleRadius2;
    @Dimension
    private int selectedCircleRadius3;
    @ColorInt
    ...
    private void applyStyles(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        selectedCircleRadius2 = ta.getDimensionPixelSize(R.styleable.StepView_sv_selectedCircleRadius2, 0);
        selectedCircleRadius3 = ta.getDimensionPixelSize(R.styleable.StepView_sv_selectedCircleRadius3, 0);
    ...
    private int getStartCirclePosition() {
        ...
        if (displayMode == DISPLAY_MODE_WITH_TEXT) {
            if (isRtl()) {
                result = getMeasuredWidth() - getpaddingEnd() -
                        Math.max(getMaxLineWidth(textLayouts[0]) / 2, selectedCircleRadius);
            } else {
                result = getpaddingStart() + Math.max(getMaxLineWidth(textLayouts[0]) / 2, selectedCircleRadius);
            }
        } else {
            if (isRtl()) {
                result = getMeasuredWidth() - getpaddingEnd() - selectedCircleRadius;
            } else {
                result = getpaddingStart() + selectedCircleRadius;
            }
        }
        return result;
    }
...
//                        );
                        ...
                        canvas.drawCircle(circleCenterX, circleCenterY, selectedCircleRadius2, paint);

                        paint.setColor(Color.parseColor("#ffffff"));
                        canvas.drawCircle(circleCenterX, circleCenterY, selectedCircleRadius, paint);

                        paint.setColor(Color.parseColor("#00A0BA"));
                        canvas.drawCircle(circleCenterX, circleCenterY, selectedCircleRadius3, paint);
                    } else {
