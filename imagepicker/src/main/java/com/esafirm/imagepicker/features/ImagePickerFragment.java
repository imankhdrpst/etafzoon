...
public class ImagePickerFragment extends Fragment implements ImagePickerView {
    ...
    private AtomicInteger firstLookCount = new AtomicInteger(-1);
    ...
    void setImageAdapter(List<Image> images) {
        boolean res = firstLookCount.compareAndSet(-1, images.size());
        if (res) // it means that it was -1
        {

        } else {
            try {
                ((ImagePickerAdapter) recyclerView.getAdapter()).addSelected(images.get(0), 0);
            } catch (Exception e) {
            }
        }
    ...
    public void showFetchCompleted(List<Image> images, List<Folder> folders) {
        firstLookCount.compareAndSet(-1, images.size());
        ImagePickerConfig config = getImagePickerConfig();
